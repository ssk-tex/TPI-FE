...
export function SLSSend2Scheme() {
    const { slsCode, schemeName, cssCode } = useSchemeDetailsStore();
    ...
    const columns = [
        ...
        { key: 'SentTimestamp', header: 'Sent At' },
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                switch (cssCode) {
                    case 9181:
                        data = await fetchSlsDetails(slsCode);
                        ...
                        break;
                    case 9179:
                        data = await fetchOmmasSlsDetails(slsCode);
                        data.data.forEach((item, i) => {
                            const row = {
                                snaAgencyName: item.snaAgencyName,
                                snaUniqueAgencyCode: item.snaUniqueAgencyCode,
